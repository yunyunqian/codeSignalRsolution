...
Every pixel x in the output image has a value equal to the average value of the 
pixel values from the 3 × 3 square that has its center at x, including x itself.
...
so there should be four integers in the blurred output. To get the first value: 
...
boxBlur <- function(image) {
  rownum = length(image)
  image = matrix(unlist(image), nrow = rownum, byrow = TRUE)
  colnum = length(image[1,])
  
  out = matrix(NA, nrow = rownum-2, ncol = colnum - 2)
  for(i in 1:(rownum-2)){
    for(j in 1:(colnum-2)){
      out[i,j] = as.integer(sum(image[i:(i+2),j:(j+2)])/9)
    }
  }
